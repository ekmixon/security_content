...
class GithubService:
    ...
    def get_changed_test_files(self):
        ...
        if branch1 != 'develop':
            branch2 = 'develop'
            differ = g.diff('--name-status', f'{branch2}...{branch1}')
            ...
            for file_path in changed_files:
                ...
                if file_path.startswith('A') or file_path.startswith('M'):
                    if (
                        'tests' in file_path
                        and not os.path.basename(file_path).startswith('ssa')
                        and os.path.basename(file_path).endswith('.test.yml')
                        and file_path not in changed_test_files
                    ):
                        changed_test_files.append(file_path)

                    # changed detections
                    if (
                        'detections' in file_path
                        and not os.path.basename(file_path).startswith('ssa')
                        and os.path.basename(file_path).endswith('.yml')
                    ):
                        file_path_base = os.path.splitext(file_path)[0].replace('detections', 'tests') + '.test'
                        file_path_new = f'{file_path_base}.yml'
