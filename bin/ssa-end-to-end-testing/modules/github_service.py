...
class GithubService:
    ...
    def __init__(self, security_content_branch):
        ...
        if os.path.exists('security_content'):
            LOGGER.warning("Found Existing Security Content Project")
        ...
        if branch1 != 'develop':
            ...
            for file_path in changed_files:
                ...
                if (
                    ...
                    and os.path.basename(file_path).startswith('ssa')
                    and file_path not in changed_ssa_test_files
                ):
                    changed_ssa_test_files.append(file_path)

                # changed detections
                if file_path.startswith('detections') and os.path.basename(
                    file_path
                ).startswith('ssa'):
